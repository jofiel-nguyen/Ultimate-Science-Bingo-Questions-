...
function updateTimerDisplay() {
    ...
    let s = timeLeft % 60;
    document.getElementById("timer").innerText = `Time Remaining: ${m.toString().padStart(2, "0")}:${s.toString().padStart(2, "0")}`;
}
...
function generateBoard() {
    ...
    for (let i = 0; i < 25; i++) {
        ...
        if (i === 12) {
            ...
            cell.innerHTML = formatChemical(answer);
            cell.dataset.raw = answer;
...
function drawQuestion() {
    ...
    document.getElementById("q-text").innerHTML = formatChemical(questionBank[random].q);
}

function handleCellClick(cell) {
    if (!gameActive || cell.classList.contains("marked")) return;
    if (cell.dataset.raw === currentAnswer) {
...
function saveScore(name, time) {
    ...
    localStorage.setItem("bingoScores", JSON.stringify(scores.slice(0, 10)));
}
...
function resetGame() {
    ...
    if (playerName.toLowerCase() === ADMIN_NAME) document.getElementById("admin-clear-btn").style.display = "block";
    generateBoard();
    startTimer();
    drawQuestion();
}
